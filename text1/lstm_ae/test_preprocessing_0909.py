...
def josa_eomi(text):
    ...
    # %cd /content/drive/My\ Drive/ai_project/데이터\ 전처리/취합본  #구글 드라이브 사용시 사용하세요
    df_stopwords = pd.read_csv('한국어_불용어.csv', encoding='utf-8')
...
def okt_test(input):
    ...
    input = spell_check(input) # hanspell 맞춤법 검사
    list = okt.morphs(input) #okt로 형태소 토큰화
    a =''
    for element in list:
        a = a+element+','
    a = a[:-1]
    input = a
    input = josa_eomi(input) #조사 어미 제거
    result = regex_check2(input) # 온점, 공백 남은것들 추가 전처리
    return result
